/* ==========================================================================
   LASERPRINT ARCADE - COROPLAST ENGINE & AUDIO SYNTHESIZER (PRODUCTION READY)
   ========================================================================== */

// Configuración de endpoints para producción y desarrollo local
...
document.addEventListener('DOMContentLoaded', () => {
  const categoria = document.body.getAttribute('data-categoria') || 'COROPLAST';
...
/**
 * Normaliza las URLs relativas del backend para apuntar al servidor Render
 */
function obtenerUrlCompleta(url) {
  if (!url) return 'https://via.placeholder.com/300x200/0d1124/00f0ff?text=SIN+ARCHIVO';
...
async function cargarProductosCategoria(categoria) {
  try {
    ...
    console.warn('⚠️ [LASERPRINT] Servidor API en pausa o no detectado. Cargando datos MOCK local:', error);
...
function renderSecciones(productos) {
  ...
  productos.forEach(prod => {
    ...
    // Reasignamos la URL procesada para consumo de los constructores
...
function createCardImagen(prod) {
  ...
  card.innerHTML = `
    ...
    <div class="card-info">
      ...
      <div class="card-price">${prod.precio ? '$' + prod.precio + ' MXN / pza' : 'COTIZAR'}</div>
    </div>
  `;
  return card;
}
...
function getDatosMOCK() {
  return [
    { nombre: 'SEÑALÉTICTA EN COROPLAST 4MM IMPRESIÓN DIRECTA', archivoUrl: '/uploads/fotos/coroplast_senal.png', tipoArchivo: 'imagen', precio: 180 },
    { nombre: 'EXHIBIDOR ARMABLE DE COROPLAST IMPRESO', archivoUrl: '/uploads/fotos/coroplast_exhibidor.png', tipoArchivo: 'imagen', precio: 450 },
    { nombre: 'PROCESO DE CORTE Y SUAJADO DIGITAL EN COROPLAST', archivoUrl: '/uploads/videos/corte_coroplast.mp4', tipoArchivo: 'video', precio: null },
    { nombre: 'PLANTILLAS Y GUÍA DE CORTE PARA COROPLAST', archivoUrl: '/uploads/docs/plantillas_coroplast.pdf', tipoArchivo: 'pdf', precio: null }
  ];
}
